...
3. 不能控制征信查询的允许时间，央行要求只能早8点至晚8点进行查询
4. 不能携带echodata——异步标识数据给每个调用方
...
2. 通过状态机和延时队列轻松实现pcm查询的时间控制和自动查询积压任务
3. 对查得的征信报告做统一的验证，防止业务端做重复工作
4. 同web页面展示pcm任务状态，并可以手动进行任务状态，优先级调整，并重新推送卡主的查询请求
# 关键设计
